...
public final class UpdateEpubPartApprovalHandler implements AgentApprovalHandler {
    ...
    public UpdateEpubPartApprovalHandler(CurrentProjectProvider currentProjectProvider, EpubPartService epubPartService) {
        this(currentProjectProvider, epubPartService, new Gson());
    }

    public UpdateEpubPartApprovalHandler(CurrentProjectProvider currentProjectProvider, EpubPartService epubPartService, Gson gson) {

        if (currentProjectProvider == null) throw new IllegalArgumentException("currentProjectProvider must not be null.");
        if (epubPartService == null) throw new IllegalArgumentException("epubPartService must not be null.");
        if (gson == null) throw new IllegalArgumentException("gson must not be null.");

        this.currentProjectProvider = currentProjectProvider;
        this.epubPartService = epubPartService;
        this.gson = gson;
    }
    ...
    public void execute(AgentApproval approval) {
        ...
        UpdateEpubPartApprovalPayload payload = parsePayload(approval);
        ...
        EpubPartPage page = new EpubPartPage(payload.getPartNumber(), payload.getFileName(), payload.getTitle(), payload.getXhtml());

        System.out.println("[GomsBook EPUB] Updating Part = " + targetFile);

        epubPartService.update(page, project.getTextDirectory());

        System.out.println("[GomsBook EPUB] Part Updated = " + targetFile);
    }
    ...
    private EpubProjectContext requireCurrentProject() {
        ...
        if (!Files.isRegularFile(project.getNavigationFile())) throw new IllegalStateException("Current EPUB navigation file does not exist: " + project.getNavigationFile());
    ...
    private UpdateEpubPartApprovalPayload parsePayload(AgentApproval approval) {

        try {

            UpdateEpubPartApprovalPayload payload = gson.fromJson(approval.getContent(), UpdateEpubPartApprovalPayload.class);
    ...
    private void validatePayload(AgentApproval approval, UpdateEpubPartApprovalPayload payload) {

    	if (payload.getPartNumber() <= 0) throw new IllegalArgumentException("partNumber must be greater than 0.");
    	if (isBlank(payload.getFileName())) throw new IllegalArgumentException("fileName must not be blank.");
    	if (isBlank(payload.getTitle())) throw new IllegalArgumentException("title must not be blank.");
    	if (isBlank(payload.getXhtml())) throw new IllegalArgumentException("xhtml must not be blank.");
    ...
    private int resolvePartNumber(String fileName) {

        Matcher matcher = PART_FILE_PATTERN.matcher(fileName.trim());

        if (!matcher.matches()) throw new IllegalArgumentException("EPUB part fileName must use the partNN.xhtml format: " + fileName);
        ...
        try {
            ...
            throw new IllegalArgumentException("Unable to resolve partNumber from fileName: " + fileName, exception);
        }
    }
    ...
    private Path resolveTargetFile(EpubProjectContext project, String fileName) {
        ...
        if (!targetFile.startsWith(textDirectory)) throw new IllegalStateException("EPUB part XHTML must be inside the EPUB Text directory.");
    ...
    private void validateTargetFile(Path targetFile) {
        ...
        if (!Files.exists(targetFile)) throw new IllegalStateException("EPUB part XHTML does not exist: " + targetFile);
        if (!Files.isRegularFile(targetFile)) throw new IllegalStateException("EPUB part XHTML path is not a file: " + targetFile);
    }
    ...
    private void validateFileName(String fileName) {
        ...
        if (!PART_FILE_PATTERN.matcher(normalized).matches()) throw new IllegalArgumentException("EPUB part fileName must use the partNN.xhtml format.");
    }
