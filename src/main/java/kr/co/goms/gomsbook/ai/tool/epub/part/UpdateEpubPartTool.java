...
public final class UpdateEpubPartTool implements AgentTool {
    ...
    private static final String DESCRIPTION =
            "Prepares an update to an existing EPUB Part XHTML file. "
                    + "Use this tool whenever the user asks to modify, edit, replace, append, rewrite, "
                    + "rename content in, or otherwise change an existing EPUB Part. "
                    + "If the current Part content is needed, call read_epub_part first. "
                    + "This tool must be called after the modified XHTML has been prepared. "
                    + "The update requires explicit user approval before any EPUB file is changed.";

    private static final String APPROVAL_TITLE =
            "EPUB 부 수정";
    ...
    public Map<String, Object> getInputSchema() {
        ...
        properties.put(
                "partNumber",
                integerProperty(
                        "수정할 EPUB 부 번호입니다. 예: 1"));

        properties.put(
                "fileName",
                stringProperty(
                        "수정할 기존 EPUB 부 XHTML 파일명입니다. 예: part01.xhtml"));
        ...
        try {
            ...
            return ToolResult.builder()
                    ...
                    .status(
                            ToolStatus.SUCCESS)
                    .message(
                            "EPUB part update approval is required.")
                    .data(
                            data)
                    .build();
            ...
            String errorMessage =
                    "Failed to prepare EPUB part update: "
                            + safeMessage(
                                    exception);
    ...
    private String createApprovalContent(
        ...
        UpdateEpubPartApprovalPayload payload =
                new UpdateEpubPartApprovalPayload(
                        page.getPartNumber(),
                        page.getFileName(),
                        page.getTitle());
